...
export type CreateStudentLeaveInput = { leaveType: string; reason: string; fromDate: string; toDate: string };

/**
 * GET/POST /api/student/leave — self-service leave request. View + create
 * only; a Student can never approve/reject (that belongs to Operations
 * leave management, an entirely different actor/surface).
 */
export function useStudentLeave() {
    ...
    async (force = false) => {
      ...
      try {
        const result = await cachedFetch(
          ...
          () => apiRequest<{ leaves: StudentLeave[] }>('/api/student/leave', {}, token).then((r) => r.leaves || []),
          { force }
        );
        setLeaves(result);
      } catch (loadError) {
  ...
  const createLeave = useCallback(
    async (input: CreateStudentLeaveInput) => {
      ...
      setCreating(true);
      try {
        await apiRequest('/api/student/leave', { method: 'POST', body: JSON.stringify(input) }, token);
